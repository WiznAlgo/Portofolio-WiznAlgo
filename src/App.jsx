...
const services = [
  ...
  },
  {
    ...
    description:
      "Menghubungkan Python, API, dan MQL4/MQL5 untuk membuat workflow trading yang lebih terukur dan minim manual task.",
  },
  {
...
const projects = [
  ...
  {
    title: "TradingView Custom Indicator",
    category: "Trading Tools",
    status: "Link Coming Soon",
    description:
      "Indikator custom untuk membantu membaca struktur market, sinyal teknikal, dan validasi setup trading. Link publik bisa ditambahkan setelah publish dari TradingView.",
    tags: ["Pine Script", "Technical Analysis", "Signal", "Market Structure"],
    live: "#tradingview-help",
    repo: "#",
    highlight: "Private indicator",
  },
  {
...
function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-zinc-950/70 backdrop-blur-2xl">
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-zinc-950/95 p-5 backdrop-blur-xl lg:hidden">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <LogoMark />
              <p className="font-bold text-white">WiznAlgo</p>
            </div>
            <button onClick={() => setOpen(false)} className="rounded-xl border border-white/10 p-3 text-white">
              <X className="h-5 w-5" />
            </button>
          </div>
          <div className="mt-10 grid gap-3">
            {navItems.map((item) => (
              <a
                ...
                className="rounded-2xl border border-white/10 bg-white/[0.03] px-5 py-4 text-lg font-semibold text-white"
              >
                {item}
              </a>
            ))}
...
function TradingViewHelp() {
  return (
    <section id="tradingview-help" className="mx-auto max-w-7xl px-5 py-24 lg:px-8">
        ...
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            ...
            <h2 className="text-3xl font-black tracking-tight text-white md:text-5xl">
              Punya indikator sendiri? Jadikan itu portfolio weapon.
            </h2>
            <p className="mt-5 leading-8 text-zinc-300">
              Setelah link TradingView sudah kamu punya, bagian ini bisa langsung diganti menjadi showcase indikator:
              fitur, screenshot, pair yang cocok, dan cara penggunaannya.
            </p>
          </div>
          ...
          <div className="rounded-[1.5rem] border border-white/10 bg-zinc-950/70 p-6">
            <h3 className="mb-5 flex items-center gap-2 text-xl font-bold text-white">
              <TrendingUp className="h-5 w-5 text-cyan-200" /> Cara ambil link TradingView
            </h3>
            <ol className="grid gap-4 text-zinc-300">
              {[
                "Buka TradingView dan masuk ke chart yang memakai indikator kamu.",
                "Klik indikator / script kamu, lalu buka halaman script jika sudah dipublish.",
                "Tekan tombol Share atau copy URL dari browser.",
                "Tempel link itu ke data project di bagian TradingView Custom Indicator.",
              ].map((step, index) => (
                <li key={step} className="flex gap-3">
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-cyan-300 text-sm font-black text-zinc-950">
                    {index + 1}
                  </span>
                  <span>{step}</span>
                </li>
...
function Contact() {
  ...
  function handleSubmit(event) {
    ...
    const message = [
      "Halo WiznAlgo, saya tertarik diskusi project.",
      "",
      `Nama: ${form.name || "-"}`,
      `Email: ${form.email || "-"}`,
